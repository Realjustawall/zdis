...
function Shell({ appName }: { appName: string }) {
  ...
  // Keep per-message array updates out of the application shell. Without this
  // selector every send/receive rerendered the rail, sidebars and active view.
  const chat = useChat(useShallow((state) => ({
    ...
    loadGroup: state.loadGroup,
    setTarget: state.setTarget,
    applyIncoming: state.applyIncoming,
    applyUpdated: state.applyUpdated,
    applyPollUpdate: state.applyPollUpdate,
    applyDeleted: state.applyDeleted,
  ...
  useEffect(() => {
    ...
    const offs = [
      ...
      on('channel:deleted', ((payload: { groupId: string; channelId: string }) => {
        ...
        if (voice.channelId === payload.channelId) voice.leave();
      }) as never),
