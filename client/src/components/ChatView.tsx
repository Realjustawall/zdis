...
export function ChatView({
  ...
  const deletedMessageIds = useChat((state) => state.deletedMessageIds);
  ...
  const [secureMode, setSecureMode] = useState(() => Boolean(
    secureStorageKey && (e2eeRequired || storageGet(secureStorageKey) === '1'),
  ));
  ...
  const scrollStateRef = useRef({ key: '', latestIdentity: null as string | null, loaded: false, count: 0 });
  ...
  useEffect(() => {
    if (secureMode && target.kind === 'conversation') {
      void prepareConversationEncryption(target.conversationId, user.id).catch(() => undefined);
    }
  }, [secureMode, target, user.id]);
  ...
  // Realtime deletions must also disappear from any currently open secondary
  // view, not only from the main message list.
  useEffect(() => {
    if (!deletedMessageIds.length) return;
    const deleted = new Set(deletedMessageIds);
    const prune = (items: Message[]) => {
      const next = items.filter((message) => !deleted.has(message.id));
      return next.length === items.length ? items : next;
    };
    setPins(prune);
    setSearchResults(prune);
    setSavedMessages(prune);
    setThreadMessages(prune);
    setSavedIds((current) => {
    ...
    setReplyTo((current) => current && deleted.has(current.id) ? null : current);
    setEditing((current) => current && deleted.has(current.id) ? null : current);
    setPollEditing((current) => current && deleted.has(current.id) ? null : current);
    setPollAnalyticsMessage((current) => current && deleted.has(current.id) ? null : current);
    setDeleting((current) => current && deleted.has(current.id) ? null : current);
    if (threadRoot && deleted.has(threadRoot.id)) {
      setThreadRoot(null);
      setThreadOpen(false);
    }
  }, [deletedMessageIds, threadRoot]);

  const scrollToBottom = useCallback(() => {
    const node = scrollRef.current;
    if (!node) return;
    atBottomRef.current = true;
    // The desktop theme enables smooth scrolling globally. Repeated height
    // changes from media can continuously restart that animation and leave the
    // viewport slightly above the bottom, so anchoring must be immediate.
    node.style.scrollBehavior = 'auto';
    node.scrollTop = node.scrollHeight;
    if (bottomFrameRef.current !== null) cancelAnimationFrame(bottomFrameRef.current);
    bottomFrameRef.current = requestAnimationFrame(() => {
      bottomFrameRef.current = null;
      if (atBottomRef.current && scrollRef.current === node) node.scrollTop = node.scrollHeight;
    });
  }, []);

  const newestMessage = messages?.[messages.length - 1];
  const newestIdentity = newestMessage
    ? (newestMessage.clientId ?? newestMessage.id)
    : null;

  // Opening a conversation always starts at the newest message. New messages
  // keep an already-pinned reader at the bottom, and messages authored by the
  // current user always return them there even if they had scrolled upward.
  useLayoutEffect(() => {
    const previous = scrollStateRef.current;
    const loaded = messages !== undefined;
    const targetChanged = previous.key !== key;
    const historyJustLoaded = !targetChanged && !previous.loaded && loaded;
    const appended = !targetChanged
      && previous.loaded
      && loaded
      && messages.length > previous.count
      && previous.latestIdentity !== newestIdentity;
    const authoredByCurrentUser = appended && newestMessage?.authorId === user.id;

    if (targetChanged || historyJustLoaded || authoredByCurrentUser || (appended && atBottomRef.current)) {
      scrollToBottom();
    }

    scrollStateRef.current = { key, latestIdentity: newestIdentity, loaded, count: messages?.length ?? 0 };
  }, [key, messages !== undefined, messages?.length, newestIdentity, newestMessage?.authorId, scrollToBottom, user.id]);

  // Media metadata, image decoding, decryption, and text previews can change a
  // row's height after the message was inserted. Preserve bottom anchoring
  // through those asynchronous layout changes without moving readers who
  // intentionally scrolled up.
  useEffect(() => {
    const list = messageListRef.current;
    if (!list || typeof ResizeObserver === 'undefined') return;
    let previousHeight = list.getBoundingClientRect().height;
    const observer = new ResizeObserver(([entry]) => {
      if (!entry) return;
      const height = entry.contentRect.height;
      const resized = Math.abs(previousHeight - height) > 0.5;
      previousHeight = height;
      if (resized && atBottomRef.current) scrollToBottom();
    });
  ...
  useEffect(() => {
    ...
    if (newest.deliveryStatus === 'sending' || newest.deliveryStatus === 'failed') return;
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [messages?.[messages.length - 1]?.id, key]);
    ...
    <div className="main">
      <div className="main-head">
        ...
        {target.kind === 'conversation' && e2eeAvailable ? (
          <button className={`head-btn e2ee-toggle${secureMode ? ' on secure' : ''}`} disabled={e2eeRequired} onClick={async () => {
            ...
            storageSet(secureStorageKey, next ? '1' : '0');
            setSecureMode(next);
            setSearchOpen(false);
      ...
      <div className="message-scroll" ref={scrollRef} onScroll={onScroll}
        ...
        onTouchMove={(event) => {
          ...
          if (currentY !== undefined && touchStartYRef.current !== null && currentY > touchStartYRef.current + 2) {
            onScrollIntent();
          }
        }}
      ...
      <Composer
        ...
        onSend={async (body) => {
          // Set this before encryption/network work so the optimistic row can
          // move the viewport immediately instead of waiting for confirmation.
          atBottomRef.current = true;
          if (secureMode && target.kind === 'conversation') {
            ...
            await sendMessage(target, { content, encrypted: true, replyToId: body.replyToId,
              ...
              optimisticAttachments: body.optimisticAttachments });
          } else {
      ...
      {lightbox ? (
        <ImageLightbox key={lightbox.id} attachment={lightbox} onClose={() => setLightbox(null)} />
      ) : null}
    </div>
  );
}
...
function ImageLightbox({ attachment, onClose }: { attachment: Attachment; onClose: () => void }) {
  ...
  return <div className="lightbox" role="dialog" aria-modal="true" aria-label={attachment.filename} onClick={onClose}>
    <div className="lightbox-toolbar" onClick={(event) => event.stopPropagation()}>
      <bdi dir="auto">{attachment.filename}</bdi>
      <a href={attachment.url} download={attachment.filename} title="Download">⭳</a>
      <button onClick={onClose} aria-label="Close">×</button>
    </div>
    {state === 'loading' ? <span className="lightbox-loading"><span className="spinner" /></span> : null}
    <img className={state === 'ready' ? 'ready' : ''} src={attachment.url} alt={attachment.filename}
      decoding="async" onLoad={() => setState('ready')} onError={() => setState('error')}
      onClick={(event) => event.stopPropagation()} />
    {state === 'error' ? <div className="media-error">Could not display this image.</div> : null}
  </div>;
}
