...
export interface EncryptedAttachmentMeta extends EncryptedFileMetadata {
  id: string;
}

type PendingAttachment = Attachment & { encryption?: EncryptedAttachmentMeta };

const TEXT_UPLOAD_EXTENSION = /\.(?:txt|md|markdown|log|csv|json|js|jsx|ts|tsx|css|py|java|c|cc|cpp|h|hpp|go|rs|sh|sql|yaml|yml)$/i;

function normalizedUploadFile(file: File) {
  if (!TEXT_UPLOAD_EXTENSION.test(file.name)) return file;
  const type = /\.json$/i.test(file.name)
    ? 'application/json'
    : /\.(?:md|markdown)$/i.test(file.name)
      ...
      : /\.csv$/i.test(file.name)
        ...
        : /\.css$/i.test(file.name)
          ? 'text/css'
          : /\.(?:js|jsx)$/i.test(file.name)
            ? 'text/javascript'
            : 'text/plain';
  return file.type === type ? file : new File([file], file.name, { type, lastModified: file.lastModified });
}

async function uploadAttachment(file: File, onProgress: (progress: number) => void) {
  ...
  const created = await api.post<{ upload: {
    id: string; chunkSize: number; totalChunks: number;
  ...
  const received = new Set(created.upload.receivedParts.map((part) => part.number));
  for (let part = 1; part <= created.upload.totalChunks; part += 1) {
    if (!received.has(part)) {
      const start = (part - 1) * created.upload.chunkSize;
      const chunk = file.slice(start, Math.min(file.size, start + created.upload.chunkSize));
      let lastError: unknown;
      for (let attempt = 0; attempt < 3; attempt += 1) {
        try {
          await api.putBinary(`/api/files/resumable/${created.upload.id}/parts/${part}`, chunk);
          lastError = null;
          break;
        } catch (error) {
          lastError = error;
    ...
    onProgress(part / created.upload.totalChunks);
  }
  const completed = await api.post<{ attachment: Attachment }>(`/api/files/resumable/${created.upload.id}/complete`);
...
export function Composer({
  ...
  useEffect(() => {
    ...
    setPending([]);
    if (secureMode) {
      setText(storageGet(`zdis.secure-draft:${targetPath}`) ?? '');
      setDraftReady(true);
      return () => { active = false; };
    }
    void api
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      if (secureMode) {
        if (text.trim()) storageSet(`zdis.secure-draft:${targetPath}`, text);
        else storageRemove(`zdis.secure-draft:${targetPath}`);
        return;
      }
  ...
  async function submit() {
    ...
    // Ordinary messages are intentionally non-blocking: snapshot and clear the
    // composer immediately so rapid consecutive sends do not wait on the API.
    if (!editing && !(command && channelId)) {
      ...
      onCancelReply();
      onStopTyping();
      if (secureMode) storageRemove(`zdis.secure-draft:${targetPath}`);
      void onSend(sendBody).catch((error) => {
        toast.error(
  ...
  async function onFiles(event: ChangeEvent<HTMLInputElement>) {
    ...
    const updateProgress = (index: number, value: number) => {
      ...
      if (value < 1 && now - lastUiUpdate < 100) return;
      lastUiUpdate = now;
      const weighted = accepted.reduce((sum, item, itemIndex) => sum + item.size * (progress.get(itemIndex) ?? 0), 0);
      const total = accepted.reduce((sum, item) => sum + item.size, 0);
      setUploadProgress(total ? weighted / total : 0);
    };
    const worker = async () => {
      ...
      try {
        const sourceFile = normalizedUploadFile(file);
        let uploadFile = sourceFile;
        let encryption: EncryptedFileMetadata | undefined;
        if (secureMode) {
          const encrypted = await encryptUploadFile(sourceFile, (value) => updateProgress(index, value * 0.35));
          uploadFile = encrypted.file;
          encryption = encrypted.metadata;
        }
        const attachment = await uploadAttachment(uploadFile, (value) =>
          updateProgress(index, secureMode ? 0.35 + value * 0.65 : value));
        setPending((current) => [...current, { ...attachment,
          encryption: encryption ? { ...encryption, id: attachment.id } : undefined }]);
      } catch (error) {
  ...
  return (
    <div className="composer-wrap">
      {uploading && uploadProgress !== null ? <div className="upload-progress" role="status">
        <span>{fa ? 'در حال آماده‌سازی و بارگذاری فایل‌ها' : 'Preparing and uploading files'}</span>
        <strong dir="ltr">{Math.round(uploadProgress * 100)}%</strong>
        <i><span style={{ width: `${uploadProgress * 100}%` }} /></i>
      </div> : null}
      {pending.length > 0 ? (
        <div className="pending-attachments">
          {pending.map((attachment) => (
            <div className="pending-chip" key={attachment.id}>
              ...
              <bdi className="name" dir="auto">{attachment.encryption?.name ?? attachment.filename}</bdi>
        ...
        {uploadsEnabled ? (
          <>
            <input
              ...
              hidden
              onChange={onFiles}
              accept="image/*,video/*,audio/*,.pdf,.zip,.txt,.md,.markdown,.log,.csv,.json,.js,.jsx,.ts,.tsx,.css,.py,.java,.c,.cpp,.h,.hpp,.go,.rs,.sh,.sql,.yaml,.yml"
            />
            <button
