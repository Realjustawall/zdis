...
export function GroupSidebar({
  ...
  const [collapsedCategories, setCollapsedCategories] = useState<Set<string>>(() => {
    try {
      return new Set(JSON.parse(localStorage.getItem(`zdis.collapsed-categories:${group.id}`) ?? '[]'));
    } catch {
      return new Set();
    }
  });
  ...
  function toggleCategory(categoryId: string) {
    setCollapsedCategories((current) => {
      ...
      localStorage.setItem(`zdis.collapsed-categories:${group.id}`, JSON.stringify([...next]));
      return next;
    });
  }
