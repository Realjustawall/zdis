...
export async function encryptConversationMessage(conversationId: string, senderId: string, plaintext: string) {
  ...
  let directory = await api.get<{ participants: Participant[] }>(`/api/e2ee/conversations/${conversationId}/participants`);
...
export async function decryptConversationMessage(content: string, conversationId: string, currentUserId: string, expectedSenderId: string) {
