...
export const useChat = create<ChatState>((set, get) => ({
  ...
  async sendMessage(target, body) {
    ...
    const reply = body.replyToId ? existing.find((message) => message.id === body.replyToId) : null;
    const optimistic: Message = {
      ...
      replyTo: reply ? {
        ...
        displayName: reply.author?.displayName ?? 'Someone',
        preview: reply.type === 'encrypted' ? 'Encrypted message' : reply.content.slice(0, 160),
        encrypted: reply.type === 'encrypted',
        encryptedContent: reply.type === 'encrypted' ? reply.content : null,
      ...
      createdAt,
      editedAt: null,
      expiresAt: body.expiresInSeconds ? createdAt + body.expiresInSeconds * 1000 : null,
      deleted: false,
      attachments: body.optimisticAttachments ?? [],
      reactions: [],
      mentionedUserIds: [],
      threadReplyCount: 0,
      poll: null,
    ...
    set((state) => ({
      messages: { ...state.messages, [key]: [...(state.messages[key] ?? []), optimistic] },
    }));

    const { optimisticAttachments: _localAttachments, ...requestBody } = body;
    try {
      const data = await api.post<{ message: Message }>(`${targetPath(target)}/messages`, {
        ...requestBody,
        clientId,
      });
      // The socket echo often arrives first; clientId reconciles either order.
          ...
          [key]: (state.messages[key] ?? []).map((message) =>
            message.id === optimisticId
              ? {
                  ...
                  deliveryError: error instanceof Error ? error.message : 'Could not send message.',
                }
              : message,
          ),
        },
  ...
  async deleteMessage(target, messageId) {
    ...
    get().applyDeleted(data.deletion ?? {
        messageId,
        channelId: target.kind === 'channel' ? target.channelId : null,
        conversationId: target.kind === 'conversation' ? target.conversationId : null,
      });
  ...
  applyIncoming(message) {
    ...
    set((state) => {
      const existing = state.messages[key] ?? [];
      const conversations = message.conversationId
        ? updateConversationPreview(state.conversations, message)
        : state.conversations;
      const matchIndex = existing.findIndex((item) =>
        item.id === message.id || Boolean(message.clientId && item.clientId === message.clientId),
      );
      if (matchIndex >= 0) {
        const matched = existing[matchIndex];
        if (matched.id === message.id && matched.deliveryStatus !== 'sending') return state;
        const reconciled = existing.slice();
        reconciled[matchIndex] = { ...message, deliveryStatus: 'sent', deliveryError: null };
        return { messages: { ...state.messages, [key]: reconciled }, conversations };
      }

      const isOpen = state.target && targetKey(state.target) === key;
      const next = { ...state.messages, [key]: [...existing, message] };

      const isOwnMessage = message.authorId === useSession.getState().user?.id;
      if (isOpen || isOwnMessage) return { messages: next, conversations };
      ...
      return { messages: next, conversations };
    });
  },
  ...
  applyDeleted(payload) {
    ...
    set((state) => {
      ...
      const removed = current.find((item) => item.id === payload.messageId);
      const nextMessages = current
        .filter((item) => item.id !== payload.messageId)
        .map((item) => {
          let next = item;
          if (item.replyTo?.id === payload.messageId) next = { ...next, replyTo: null };
          if (removed?.replyToId === item.id && (item.threadReplyCount ?? 0) > 0) {
            next = { ...next, threadReplyCount: (item.threadReplyCount ?? 0) - 1 };
          }
          return next;
        });
      const conversations = payload.conversationId
        ? state.conversations
            .map((conversation) =>
              conversation.id === payload.conversationId
                ...
                ? {
                    ...conversation,
                    updatedAt: payload.conversationUpdatedAt
        ...
        : state.conversations;
      const deletedMessageIds = state.deletedMessageIds.includes(payload.messageId)
        ? state.deletedMessageIds
        : [...state.deletedMessageIds, payload.messageId].slice(-500);
      return {
        messages: { ...state.messages, [key]: nextMessages },
        conversations,
        deletedMessageIds,
      };
    });
...
function dedupe(messages: Message[]): Message[] {
  ...
  for (const message of messages) {
    if (seen.has(message.id) || Boolean(message.clientId && seenClients.has(message.clientId))) continue;
    seen.add(message.id);
    if (message.clientId) seenClients.add(message.clientId);
    out.push(message);
  }
  return out.sort((a, b) => a.createdAt - b.createdAt || (a.id < b.id ? -1 : a.id > b.id ? 1 : 0));
}

function updateConversationPreview(conversations: Conversation[], message: Message) {
