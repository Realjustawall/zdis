...
import { authorizedObjectUrl, readObjectBuffer, readObjectRange } from '../services/storage.js';
...
filesRouter.get(
  ...
  asyncRoute(async (req, res) => {
    ...
    if (range && req.params.variant === 'optimized') {
    ...
    res.setHeader('Accept-Ranges', req.params.variant === 'optimized' ? 'bytes' : 'none');
...
filesRouter.get(
  ...
  asyncRoute(async (req, res) => {
    ...
    const isTextPreview = attachment.mime.startsWith('text/') || attachment.mime === 'application/json';
    const responseMime = isTextPreview ? 'text/plain; charset=utf-8' : attachment.mime;
    ...
    const supportsRange = isInline || attachment.mime === 'application/vnd.zdis.encrypted';
    const disposition =
    ...
    if (attachment.mime === 'application/vnd.zdis.encrypted' &&
        (attachment.storage_provider ?? 'local') === 's3') {
      ...
      if (match) {
        ...
        if (Number.isSafeInteger(start) && Number.isSafeInteger(end) && start >= 0 && start <= end && end < size) {
          const bytes = await readObjectRange(attachment, start, end);
          res.status(206);
    ...
    // Text previews are fetched by JavaScript, so proxy object-storage bytes
    // through the authenticated origin instead of depending on bucket CORS.
    if (isTextPreview && (attachment.storage_provider ?? 'local') === 's3') {
      const size = Number(attachment.size);
