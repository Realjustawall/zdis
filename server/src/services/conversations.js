...
export async function listConversations(userId) {
  ...
  const lastRows = await db.all(
    `SELECT m.conversation_id, m.id, m.content, m.type, m.author_id, m.created_at, m.deleted_at,
            u.display_name AS author_name
     FROM messages m
     JOIN users u ON u.id = m.author_id
     WHERE m.conversation_id IN (${placeholders})
       AND m.id = (SELECT MAX(id) FROM messages WHERE conversation_id = m.conversation_id)`,
    ids,
  );
  ...
  return rows.map((row) => {
    ...
    return toConversation(row, {
      ...
      lastMessage: last
        ? {
            id: last.id,
            preview: last.deleted_at
              ? 'Message deleted'
              : last.type === 'encrypted'
                ? 'Encrypted message'
                : String(last.content).slice(0, 120),
            encrypted: !last.deleted_at && last.type === 'encrypted',
            encryptedContent: !last.deleted_at && last.type === 'encrypted' ? last.content : null,
