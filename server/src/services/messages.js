...
export function toMessage(row, extras = {}) {
  ...
  return {
    id: row.id,
    clientId: row.client_nonce ?? null,
...
export async function createMessage({
  ...
  const db = getDb();
  // A client nonce makes POST retries and the REST/socket race idempotent.
  // It is scoped to the author by a unique index and contains no message data.
  if (clientId) {
    const existing = await db.get(
      'SELECT id FROM messages WHERE author_id = ? AND client_nonce = ?',
      [authorId, clientId],
    );
...
async function attachExtras(rows, viewerId = null) {
  ...
  // These datasets are independent. Parallel reads save five network round
  // trips on PostgreSQL while preserving the same bounded page queries.
  const [attachmentRows, reactionRows, mentionRows, threadRows, pollRows, replyRows] = await Promise.all([
    db.all(`SELECT * FROM attachments WHERE message_id IN (${placeholders})`, ids),
    db.all(`SELECT message_id, emoji, user_id FROM reactions WHERE message_id IN (${placeholders})`, ids),
    db.all(`SELECT message_id, user_id FROM mentions WHERE message_id IN (${placeholders})`, ids),
    db.all(
    ...
    db.all(
      `SELECT p.*, po.id AS option_id, po.label AS option_label, po.position,
              po.is_correct AS option_correct, pv.user_id AS vote_user_id
       FROM polls p
    ...
    replyIds.length
      ? db.all(
          `SELECT m.id, m.content, m.type, m.author_id, m.conversation_id, m.deleted_at,
           ...
           WHERE m.id IN (${replyPlaceholders}) AND m.deleted_at IS NULL`,
          replyIds,
        )
      : Promise.resolve([]),
  ]);
  const replyMap = new Map(
    replyRows.map((row) => [
        row.id,
        {
          ...
          preview: row.type === 'encrypted' ? 'Encrypted message' : String(row.content).slice(0, 160),
...
export function toAttachment(row) {
  return {
    ...
    optimizedUrl: row.derived_key ? `/api/files/${row.id}/optimized` : null,
