...
export async function completeResumableUpload(id, userId) {
  ...
  try {
    // Assemble into one destination buffer instead of retaining every part and
    // then Buffer.concat-ing a second full-size copy.
    const buffer = Buffer.allocUnsafe(Number(upload.total_size));
    const hasher = crypto.createHash('sha256');
    let offset = 0;
    for (let number = 1; number <= Number(upload.total_chunks); number += 1) {
      const part = await fs.readFile(partPath(id, number));
