...
export const ALLOWED_TYPES = {
  ...
  'text/css': { ext: 'css', category: 'document' },
  'text/javascript': { ext: 'js', category: 'document' },
...
export function sniffMime(buffer, declared) {
  if (declared === 'application/vnd.zdis.encrypted' &&
      buffer.length > 8 && ['ZDISENC1', 'ZDISENC2'].includes(buffer.subarray(0, 8).toString('ascii'))) {
    return declared;
  }
  ...
  if ((declared === 'application/json' ||
      ['text/plain', 'text/markdown', 'text/csv', 'text/css', 'text/javascript'].includes(declared)) &&
...
function isProbablyText(buffer) {
  ...
  try {
    ...
    const suspiciousControls = [...decoded].filter((char) => {
      const code = char.charCodeAt(0);
      return code < 32 && !['\n', '\r', '\t'].includes(char);
    }).length;
    return suspiciousControls <= Math.max(2, decoded.length * 0.002);
  } catch {
    return false;
  }
}
...
export async function storeUpload({
  ...
  try {
    ...
    if (detected.startsWith('text/') || detected === 'application/json') {
