...
await check('message client nonce is idempotent', async () => {
  const clientId = `client${Math.random().toString(36).slice(2).padEnd(26, '0')}`.slice(0, 32);
  const body = { content: 'one logical optimistic message', clientId };
  const first = await creator.post(`/api/channels/${textChannel.id}/messages`, body);
  const replay = await creator.post(`/api/channels/${textChannel.id}/messages`, body);
  assert(first.status === 201, `expected initial 201, got ${first.status}`);
  assert(replay.status === 200, `expected replay 200, got ${replay.status}`);
  assert(replay.body.message.id === first.body.message.id, 'replay created a second message id');
  assert(replay.body.message.clientId === clientId, 'client nonce was not returned for reconciliation');
  const list = await creator.get(`/api/channels/${textChannel.id}/messages`);
  assert(list.body.messages.filter((item) => item.clientId === clientId).length === 1, 'nonce replay duplicated history');
});
...
await check('deleted messages and deleted reply quotes are absent from history', async () => {
  const parent = await creator.post(`/api/channels/${textChannel.id}/messages`, { content: 'temporary parent' });
  const reply = await ed.post(`/api/channels/${textChannel.id}/messages`, {
    content: 'reply remains', replyToId: parent.body.message.id,
  });
  const removed = await creator.del(`/api/channels/${textChannel.id}/messages/${parent.body.message.id}`);
  assert(removed.body.deletion?.messageId === parent.body.message.id, 'delete response lacks reconciliation payload');
  const history = await creator.get(`/api/channels/${textChannel.id}/messages`);
  assert(!history.body.messages.some((item) => item.id === parent.body.message.id), 'deleted row remained in history');
  const remainingReply = history.body.messages.find((item) => item.id === reply.body.message.id);
  assert(remainingReply, 'reply should remain after its parent is deleted');
  assert(remainingReply.replyTo === null, 'reply exposed a deleted-message placeholder');
  assert(!history.body.messages.some((item) => item.deleted), 'history exposed a deletion tombstone');
});
...
await check('deleting the latest DM restores the previous sidebar preview', async () => {
  await admin.patch('/api/admin/settings', { e2ee_required_for_dms: false });
  try {
    const first = await creator.post(`/api/conversations/${dm.id}/messages`, { content: 'previous private preview' });
    const latest = await creator.post(`/api/conversations/${dm.id}/messages`, { content: 'temporary private preview' });
    const removed = await creator.del(`/api/conversations/${dm.id}/messages/${latest.body.message.id}`);
    assert(removed.body.deletion?.lastMessage?.id === first.body.message.id, 'delete event did not carry the replacement preview');
    const conversations = await creator.get('/api/conversations');
    const listed = conversations.body.conversations.find((item) => item.id === dm.id);
    assert(listed?.lastMessage?.id === first.body.message.id, 'sidebar preview still points at the deleted message');
    assert(listed?.lastMessage?.preview === 'previous private preview', 'sidebar did not restore the previous preview');
    const history = await creator.get(`/api/conversations/${dm.id}/messages`);
    assert(!history.body.messages.some((item) => item.id === latest.body.message.id), 'deleted DM remained in history');
  } finally {
    await admin.patch('/api/admin/settings', { e2ee_required_for_dms: true });
  }
});
